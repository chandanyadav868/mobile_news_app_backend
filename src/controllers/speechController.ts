...
export class SpeechController {
     ...
     * Seamlessly routed to Microsoft Edge TTS neural engine (100% pure Node.js, zero Python overhead)
     */
    public static async synthesizeSano(req: Request, res: Response): Promise<void> {
        try {
            ...
            const startTime = Date.now();
            const detected = TTSService.detectLanguageFromText(text);
            const chosenVoice = detected.lang !== 'en' ? detected.voice : 'en-IN-NeerjaNeural';

            const speechData = await TTSService.getSpeechAudio(text.trim(), chosenVoice);
            const elapsedMs = Date.now() - startTime;
            ...
            res.status(200).json({
                ...
                cached: speechData.cached,
                latencyMs: elapsedMs,
                engine: 'Edge-TTS (High-Speed Neural)',
            });
        } catch (err: any) {
            console.error('TTS synthesize error:', err);
            res.status(500).json({ success: false, error: err.message || 'TTS synthesis failed' });
        }
    }
}
